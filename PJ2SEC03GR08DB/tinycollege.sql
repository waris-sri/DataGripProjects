...
INSERT INTO `class` (`CLASS_CODE`, `COURSE_CODE`, `CLASS_SECTION`, `CLASS_TIME`, `CLASS_ROOM`, `EMP_NUM`)
VALUES ('10012', 'ACCT-211', '1', 'MWF 8:00-8:50 a.m.', 'KLR 225', 105),
...
INSERT INTO `course` (`COURSE_CODE`, `COURSE_NAME`, `CRS_CREDIT`, `DEPT_CODE`)
VALUES ('ACCT-211', 'Accounting I', 3, 'ACCT'),
...
INSERT INTO `department` (`DEPT_CODE`, `DEPT_NAME`, `EMP_NUM`, `DEPT_ADDRESS`, `DEPT_EXTENSION`)
VALUES ('ACCT', 'Accounting', 114, 'KLR 211, Box 52', '3119'),
...
INSERT INTO `enroll` (`CLASS_CODE`, `STU_NUM`, `GRADE`, `CREDIT`)
VALUES ('10012', 311198, 'A', 3),
       ('10012', 311357, 'A', 3),
       ('10012', 322345, 'A', 3),
       ('10012', 324274, 'A', 3),
       ('10012', 343749, 'B', 3),
...
VALUES (103, 'Ronald', 'Donelly', '1952-10-02 00:00:00', 'HIST', '6783'),
...
VALUES (311198, 'Anne', 'Robertson', '1970-11-15', '3', '3.04', 'CIS', 162),
...
-- List departments that have their student average GPA of more than 3.00, group ascendingly by department code
...
--     Student
-- GROUP BY dept_code
-- HAVING AVG(stu_gpa) > 3.0
-- ORDER BY dept_code ASC;

-- Cross join 2 tables

-- SELECT * FROM department; -- 11 rows
-- SELECT * FROM course; -- 28 rows
-- SELECT * FROM department CROSS JOIN course; -- 308 rows
-- SELECT * FROM course CROSS JOIN department; -- 308 rows

--

-- SELECT 
--     c.EMP_NUM, p.EMP_FNAME, p.EMP_LNAME
-- FROM
--     class c
--         RIGHT OUTER JOIN
--     professor p ON p.EMP_NUM = c.EMP_NUM
-- WHERE
--     c.EMP_NUM IS NULL
-- GROUP BY p.EMP_NUM;
...
# FROM class c
#          INNER JOIN
#      course crs ON c.COURSE_CODE = crs.COURSE_CODE
#          LEFT JOIN
#      enroll e ON c.CLASS_CODE = e.CLASS_CODE
# WHERE e.CLASS_CODE IS NULL
# ORDER BY crs.COURSE_NAME;

--
-- In-class Exercise: SQL Part 3 (Lecture 13)
--

-- Q1
SELECT s.STU_FNAME, s.STU_LNAME, e.CLASS_CODE, cl.CLASS_CODE, c.COURSE_NAME, e.GRADE
FROM student s
         INNER JOIN enroll e ON s.STU_NUM = e.STU_NUM
         INNER JOIN class cl ON cl.CLASS_CODE = e.CLASS_CODE
         INNER JOIN course c ON c.COURSE_CODE = cl.COURSE_CODE
WHERE c.COURSE_CODE LIKE 'CIS%'
ORDER BY c.COURSE_CODE, e.GRADE;

-- Q2
SELECT CONCAT(EMP_FNAME, ' ', EMP_LNAME) AS contact, PROF_EXTENSION AS extension
FROM professor
UNION
SELECT DEPT_NAME AS contact, DEPT_EXTENSION AS extension
FROM department d
         INNER JOIN professor p ON p.EMP_NUM = d.EMP_NUM;

-- Q3
SELECT d.DEPT_CODE, d.DEPT_NAME, c.COURSE_CODE, c.COURSE_NAME, COUNT(CLASS_CODE) AS 'Num. of Classes'
FROM department d
         INNER JOIN course c ON d.DEPT_CODE = c.DEPT_CODE
         LEFT OUTER JOIN class cl ON cl.COURSE_CODE = c.COURSE_CODE
...
-- Q5
SELECT STU_NUM, STU_FNAME, STU_LNAME, s.EMP_NUM, EMP_FNAME, EMP_LNAME
FROM student s
         INNER JOIN professor p ON s.EMP_NUM = p.EMP_NUM
         INNER JOIN department d ON s.EMP_NUM = d.EMP_NUM;
...
FROM student
WHERE ( YEAR(CURDATE()) - YEAR(STU_DOB) ) >
      (
          SELECT AVG(YEAR(CURDATE()) - YEAR(STU_DOB)) AS avg_age FROM student WHERE MONTH(STU_DOB) = 12
          )
ORDER BY age DESC;
...
WHERE STU_GPA > (
                    SELECT AVG(STU_GPA)
                    FROM student
                    WHERE DEPT_CODE LIKE 'MATH'
                    )
ORDER BY STU_GPA DESC;

-- Q8 (Hard)
-- Calculate the average number of class per instructors.
SELECT AVG(num) AS 'Average Num. of Teaching Classes per Prof.'
FROM (
         SELECT COUNT(CLASS_CODE) AS num
         FROM class c
...
-- Q9 (Hard)
-- List all the students who rank top in each department, including stu_num, stu_fname, stu_lname, stu_gpa, and dept_code.
SELECT s.STU_NUM, s.STU_FNAME, s.STU_LNAME, s.STU_GPA, s.DEPT_CODE
FROM student s
         INNER JOIN (
                        SELECT DEPT_CODE, MAX(STU_GPA) AS max_gpa
                        FROM student
                        GROUP BY DEPT_CODE
                        ) m ON s.DEPT_CODE = m.DEPT_CODE AND s.STU_GPA = m.max_gpa
...
-- The query above will have the exact same result when run `SELECT * FROM vw_topstudents;`